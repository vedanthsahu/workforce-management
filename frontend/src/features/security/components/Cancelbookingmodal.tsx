...
// export function CancelBookingModal({ visitor, onClose, onSuccess }: Props) {
...
//                 {visitor.bookingDate} · {visitor.visitTimeLabel}
//               </p>
//             </div>
//           </div>
...
export function CancelBookingModal({ visitor, onClose, onSuccess }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await securityService.cancelVisit(visitor.guestVisitId, {
        cancellation_reason: trimmed,
      });
      onSuccess();
      onClose();
