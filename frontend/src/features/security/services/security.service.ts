...
//     site_id?: string;
//   }): Promise<ApiSecurityDashboardSummary> {
//     const { data } = await axiosInstance.get("/security/dashboard/summary", { params });
//     return data;
//   },
...
//   // ── GET ALL GUEST BOOKINGS (with client-side search + pagination) ──────────
//   async getVisitors(filters?: VisitorFilters): Promise<VisitorResponse> {
//     const params: Record<string, unknown> = { limit: 200 }; // fetch enough for client-side ops

//     // Map status filter to booking_status query param
//     if (filters?.status && filters.status !== "ALL") {
//       // The API uses booking_status; visit_status is on the record itself.
//       // We'll filter client-side by visit_status to keep flexibility.
//     }
...
//     // Filter by guest name OR host name (case-insensitive)
//     if (filters?.search?.trim()) {
//       const q = filters.search.trim().toLowerCase();
//       items = items.filter(
//         (v) =>
...
export const securityService = {
  ...
  async getVisitors(filters?: VisitorFilters): Promise<VisitorResponse> {
    // API max limit is 100; we fetch the full page in one shot then paginate client-side
