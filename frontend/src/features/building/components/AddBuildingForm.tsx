...
// };
  const handleSave = async () => {
    ...
    if (success) {
      ...
      // router.push is correct here — this is a post-action redirect,
      // not a user-initiated navigation, so Link can't be used.
      // prefetch() was already called on hover of the Save button (see below).
      setTimeout(() => {
        router.push("/admin/building");
      }, 1000);
    }
  ...
  return (
    <div className="p-6 space-y-6 bg-[#f8fafc] min-h-screen">
      ...
      {/*
        ✅ BACK — pure navigation, no action needed → use <Link>.
        Next.js will prefetch /admin/building as soon as this renders.
      */}
      <Link
        href="/admin/building"
        className="inline-flex items-center gap-1.5 text-xs font-medium text-gray-500 hover:text-gray-900 mb-5 transition-colors"
    ...
    {errorMessage}
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5">
        ...
        <div className="flex items-center gap-2 sm:shrink-0">
          ...
          {/*
            ✅ CANCEL — pure navigation → use <Link>.
          */}
          <Link
            href="/admin/building"
          ...
          {/*
            ✅ SAVE — post-action redirect, must stay as button.
            onMouseEnter prefetches /admin/building so the redirect
            after save is instant (bundle already downloaded).
          */}
          <button
            onClick={handleSave}
