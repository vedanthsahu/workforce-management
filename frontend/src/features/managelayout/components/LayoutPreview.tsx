...
//   // ── Compute colored SVG — useMemo so it reacts to seat changes ─────────
//   const filteredIds = useMemo(
//     () => filteredSeats ? new Set(filteredSeats.map((s) => s.seat_svg_id)) : undefined,
//     [filteredSeats]
//   );
// // Add right before the coloredSvg useMemo
// useEffect(() => {
//   console.log("[LayoutPreview] seats prop changed, count:", seats.length, 
//     "configured count:", seats.filter(s => s.is_configured).length);
// }, [seats]);
//   // Key insight: seats array reference changes when Zustand updateSeat runs,
//   // so this useMemo recomputes instantly without needing a page refresh.
...
//     ${fillHeight ? "flex-1 min-h-0" : "h-[320px] sm:h-[400px] md:h-[460px]"}`}
//   style={fillHeight ? { width: "100%" } : { width: "100%" }}
// >
//           {mapReady && (
//             <div className="absolute top-3 right-3 z-20 flex flex-col gap-1.5">
//               <button onClick={(e) => { e.stopPropagation(); fitView(); }} title="Fit to view"
