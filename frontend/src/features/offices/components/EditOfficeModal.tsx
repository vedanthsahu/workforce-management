...
type Props = {
  ...
  open: boolean;
  onClose: () => void;
  onSuccess: (siteId: string) => void; // ← now accepts siteId
};

export default function EditOfficeModal({ office, open, onClose, onSuccess }: Props) {
  const [loading, setLoading] = useState(false);
  ...
  //     await onSuccess(); // then refresh + show page banner
  //   } catch (err) {
  //     console.error(err);
  //   } finally {
  //     setLoading(false);
  ...
  try {
    ...
    await officeService.updateSite(office.site_id, formData);
    onClose();
    await onSuccess(office.site_id); // ← pass site_id back
  } catch (err) {
    console.error(err);
  } finally {
    setLoading(false);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm px-4">
      ...
      <div className="w-full max-w-2xl rounded-3xl bg-white shadow-2xl overflow-hidden">
        ...
        <div className="flex items-center justify-between border-b px-6 py-5">
          <div>
            <h2 className="text-xl font-semibold text-gray-900">Edit Office</h2>
            <p className="text-sm text-gray-500 mt-1">Update office details and save changes.</p>
          </div>
