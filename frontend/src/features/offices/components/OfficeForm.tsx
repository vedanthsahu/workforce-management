...
//   try {
//     const result = await createSite(formData); // ← capture the result
//     setSuccessMessage("Office added successfully!");
//     setFormData({
//       site_name: "",
//       site_code: "",
...
//     });
//     setTimeout(() => {
//       router.push(`/admin/offices?success=true&site_id=${result.site_id}`); // ← pass site_id
//     }, 1500);
//   } catch (error) {
//     console.error(error);
//   }
...
export default function OfficeForm() {
  ...
  // ← Removed successMessage, added errorMessage
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    setErrorMessage(""); // clear error on any change
    setFormData((prev) => ({
      ...prev,
      [e.target.name]: e.target.value,
    }));
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // No success message here — banner shows on the list page
      setFormData({
        site_name: "",
        site_code: "",
        city: "",
