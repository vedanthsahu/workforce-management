...
export function useBookingForm() {
  ...
  // ── Re-fetch seats on step 2 page refresh ────────────────────────────────
  // IMPORTANT: Guard on availablePreferences.length > 0 so that resolveAmenityIds
  // has the full list before running. Without this guard, preferences and the
  // seats fetch race each other on reload, causing amenity IDs to resolve as []
  // and seat colours (which depend on matched amenities) to be lost.
  ...
  useEffect(() => {
    if (
      ...
      form.fromDate &&
      form.toDate &&
      availablePreferences.length > 0   // ← wait for preferences to load first
    ) {
      setLoadingSeats(true);
      const amenityIds = resolveAmenityIds(form.preferences);
      fetchSeatsWithAvailability({
  ...
  // availablePreferences added to deps so the effect re-runs once they arrive
  // eslint-disable-next-line react-hooks/exhaustive-deps
