...
//                 <tr
//                   key={amenity.amenity_id}
//                   className={isHighlighted ? "row-highlight" : "hover:bg-gray-50"}
//                 >
//                   {/* AMENITY NAME */}
//                   <td className="px-3 py-3">
//                     <div className="flex items-center gap-2">
...
import { Pencil, Wifi, Coffee, Monitor, VolumeX, Building, DoorOpen, Tag } from "lucide-react";
import { Amenity } from "../types/amenities.types";
...
const iconMap: Record<string, any> = {
  wifi: Wifi, coffee: Coffee, monitor: Monitor,
  "volume-x": VolumeX, elevator: Building, window: DoorOpen,
};
            ...
            {data.map((amenity) => {
              ...
              return (
                <tr key={amenity.amenity_id} className={isHighlighted ? "row-highlight" : "hover:bg-gray-50"}>
                  <td className="px-3 py-3">
                    <div className="flex items-center gap-2">
                      <div className={`w-7 h-7 flex-shrink-0 flex items-center justify-center rounded-full ${isHighlighted ? "bg-blue-200" : "bg-blue-100"}`}>
                        <Icon className="w-3 h-3 text-blue-600" />
                      </div>
                      <div>
                        <p className="font-medium text-gray-900">{amenity.amenity_name}</p>
