...
// // Update these to match the `name` field values your /preferences endpoint returns.
...
//   // ── Modify handler — same pattern as MyBookingsPage ───────────────────────
//   // If the booking has real preference keys from the API, use them directly.
//   // Otherwise pass fallback display names so useBookingForm can resolve them
//   // to real keys once the /preferences API responds.

//   const handleModifyBooking = async (booking: Booking) => {
//     if (!canBookSelf) return;

//     const realPrefKeys: string[] = (booking as any).preferences ?? [];
//     const hasRealPrefs = realPrefKeys.length > 0;

//     // If no preference keys, attempt to fetch amenities from the seat API.
//     // If that also returns nothing, we fall through to the name-based fallback.
//     let resolvedKeys: string[] = realPrefKeys;
//     if (!hasRealPrefs && booking.floorId && booking.rawSeatId) {
//       try {
...
//       // Real keys available — pass directly
//       params.set("preferences", resolvedKeys.join(","));
//     } else {
//       // Nothing from the API — pass fallback display names for resolution
      ...
      <div className="rounded-2xl bg-gradient-to-br from-indigo-600 via-indigo-600 to-indigo-700 px-5 py-6 flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 relative overflow-hidden animate-fade-in">
        ...
        {canBookSelf && (
          <Button
            size="sm"
            onClick={onBookNow}
            className="bg-white text-indigo-700 hover:bg-indigo-50 hover:scale-[1.03] active:scale-[0.97] transition-all duration-150 text-[11.5px] font-semibold shrink-0 h-[32px] px-4 rounded-xl shadow-md shadow-indigo-900/20 border-0 self-start sm:self-auto z-10 cursor-pointer"
          >
            Book Now →
          </Button>
        )}
...
//       <div className="group bg-white border border-violet-100 rounded-xl p-3 relative overflow-hidden col-span-2 sm:col-span-1">
//         <div className="absolute top-0 left-0 right-0 h-[2px] bg-gradient-to-r from-violet-500 to-transparent" />
//         <div className="flex items-center justify-between mb-2">
//           <p className="text-[11px] text-gray-500 font-medium uppercase tracking-wide">Office visits</p>
//           {/* <div className="w-7 h-7 rounded-lg bg-violet-100 flex items-center justify-center shrink-0"> */}
...
export default function DashboardPage() {
  ...
  const handleModifyBooking = async (booking: Booking) => {
    ...
    // If no preference keys, attempt to fetch amenities from the seat API.
    // Any error (including 409 conflict) is silently swallowed — we fall
    // through to the name-based fallback in that case.
    let resolvedKeys: string[] = realPrefKeys;
    if (!hasRealPrefs && booking.floorId && booking.rawSeatId) {
      try {
        const amenities = await fetchSeatAmenities(
          booking.floorId,
          booking.rawSeatId,
          booking.fromDate ?? booking.date,
        );
        resolvedKeys = amenities ?? [];
      } catch (e: unknown) {
        // 409, 404, network errors — all fall through to preferenceNames fallback
        resolvedKeys = [];
      }
    }
