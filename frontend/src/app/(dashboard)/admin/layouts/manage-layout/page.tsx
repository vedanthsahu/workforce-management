...
export default function ManageLayoutPage() {
  ...
  const handleSiteChange     = (id: string) => setSelectedSiteId(id);
  const handleBuildingChange = (id: string) => setSelectedBuildingId(id);
  const handleFloorChange    = (id: string) => setSelectedFloorId(id);
  const handleLayoutChange   = (id: string) => { setSelectedLayoutId(id); syncUrl({ layoutId: id }); };

  return (
    <div className="flex h-screen w-full">
      <div className="flex flex-col flex-1 min-w-0">

        {/* FIX: reduced padding on mobile, full padding on sm+ */}
        <main className="flex-1 bg-gray-50 px-3 py-4 sm:p-6 space-y-4 sm:space-y-5 overflow-y-auto">
          ...
          <div className="flex flex-col lg:flex-row gap-4 lg:gap-6 w-full items-start">
            ...
            {/* SVG preview — full width on mobile, flex-1 on desktop */}
