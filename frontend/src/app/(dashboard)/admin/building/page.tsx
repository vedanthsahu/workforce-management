...
export default function BuildingsPage() {
  ...
  return (
    <div className="p-6 space-y-6 bg-[#f8fafc] min-h-screen">
      ...
      <div className="text-sm text-gray-500">
        Admin / Buildings /{" "}
        <span className="text-gray-800">Manage Buildings</span>
      </div>
      ...
      <div className="flex justify-between items-center">
        ...
        {/*
          ✅ Replaced <button onClick={() => router.push(...)}> with <Link>.
          Next.js prefetches /admin/building/add as soon as this button
          enters the viewport, so the page is ready before the user clicks.
          useRouter and the router.push call are no longer needed here.
        */}
        <Link
      ...
      <div className="bg-white border border-gray-200 rounded-2xl shadow-sm flex flex-col">
        ...
        >
          {loading ? (
            ...
            <BuildingTable data={paginatedBuildings} onEdit={handleEdit} />
          )}
        </div>
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between px-4 sm:px-6 py-4 border-t shrink-0 text-xs sm:text-sm text-gray-500">
          <span>
            Showing{" "}
            {filteredBuildings.length === 0 ? 0 : startIndex + 1}{" "}
            to{" "}
            {Math.min(startIndex + itemsPerPage, filteredBuildings.length)}{" "}
            of {filteredBuildings.length} entries
          </span>
          <div className="self-center sm:self-auto">
            <BuildingPagination
      ...
      {selectedBuilding && (
        <EditBuildingModal
          ...
          onSuccess={() => fetchBuildings(selectedSiteId)}
        />
      )}
